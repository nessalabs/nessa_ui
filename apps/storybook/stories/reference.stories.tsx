...
function AnswerParagraph() {
  return (
    <div className="w-[min(42rem,calc(100vw-2rem))] rounded-3xl border border-border bg-background p-8">
      <p className="m-0 font-sans text-base leading-7 text-foreground">
        ...
        <Reference>
          <ReferenceTrigger
            ...
            aria-label="Source: Stripe Investor Letter, page 14"
          >
            <GlobeIcon />2
          </ReferenceTrigger>
          <ReferenceContent aria-label="Cited sources">
            <ReferenceCard
              sources={[investorLetter, embeddedFinanceReport]}
            />
          </ReferenceContent>
        </Reference>{" "}
        ...
        <Reference>
          ...
          <ReferenceContent aria-label="Cited source">
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "Inline citation for agent answers and research surfaces: a chip embedded in flowing text reveals its supporting evidence in a floating card on hover or keyboard focus, and clicking follows the source. The card takes the batteries-included ReferenceCard — source title, quoted excerpt, locator chip, source link, and a pager when a claim cites several sources — or any custom node the host supplies.",
      },
    },
  },
...
export const CitedAnswer: Story = {
  parameters: storyDocumentation(
    "An agent answer with citation chips inline in the prose. Hovering a chip floats the source card above it — title, quoted excerpt, page locator, and an explicit source link — while clicking the chip itself follows the citation directly.",
  ),
  render: () => <AnswerParagraph />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    const trigger = canvas.getByRole("link", {
      name: "Source: Stripe Investor Letter, page 14",
    })
    ...
    await expect(
      portal.getByText("Stripe Investor Letter"),
    ).toBeVisible()
...
export const MultipleSources: Story = {
  parameters: storyDocumentation(
    "One claim, several sources: the header grows a pager. Arrows step through the cited documents, the count announces politely for screen readers, and the ends disable instead of wrapping so paging never loops silently. While a pager is present the excerpt region holds a fixed height and scrolls its overflow, so stepping between long and short quotes never shifts the card; hosts retune the height through ReferenceCard's excerptClassName.",
  ),
  render: () => (
    <div className="flex min-h-80 w-[min(42rem,calc(100vw-2rem))] items-end justify-center rounded-3xl border border-border bg-background p-8 pb-10">
      <p className="m-0 font-sans text-base leading-7 text-foreground">
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => expect(openCard()).not.toBeNull())
    const portal = cardCanvas()
    await expect(portal.getByText("1 of 2")).toBeVisible()
    const card = openCard()!
    ...
    await expect(portal.getByText("2 of 2")).toBeVisible()
    await expect(
...
export const KeyboardAccess: Story = {
  parameters: storyDocumentation(
    "The chip is a real link in the tab order: focusing it opens the card without a pointer, Escape dismisses it, and the citation stays followable because the chip itself carries the href.",
  ),
  render: () => (
    <div className="flex min-h-72 w-[min(42rem,calc(100vw-2rem))] items-end justify-center rounded-3xl border border-border bg-background p-8 pb-10">
      <p className="m-0 font-sans text-base leading-7 text-foreground">
    ...
    <div className="flex min-h-80 w-[min(42rem,calc(100vw-2rem))] items-end justify-center rounded-3xl border border-border bg-background p-8 pb-10">
      <p className="m-0 font-sans text-base leading-7 text-foreground">
        ...
        <Reference>
          ...
          <ReferenceContent aria-label="Paper preview">
