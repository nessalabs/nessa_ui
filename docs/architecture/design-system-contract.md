...
- A borderless editable surface nested inside a bordered container of its own — a composer's textarea, a draft field on a card — draws no focus outline. Browsers match `:focus-visible` on editable elements for pointer focus as well as keyboard focus, so an outline there reads as a permanent inner border rather than as focus, while the caret and selection already say where focus is. This is narrow: a standalone bordered field (Input) keeps its ring, and a non-editable region that earns a tab stop by scrolling — a capped payload, a read-only draft — draws one, because it has no caret to indicate focus with. A component that omits the outline records that choice in the focus inventory rather than leaving the absence to be read as an oversight.
...
| A11Y-005 | Editable surfaces and live regions follow the frozen focus and announcement rules. | `#accessibility-and-rendering-invariants` |
