...
- Focus indication belongs to whichever element reads as the field. Input is itself the field and draws its ring; a composer's textarea or a draft inside a card is the interior of a surface that is the field, so an outline on the interior reads as a permanent inner border rather than as focus — and browsers match `:focus-visible` on editable elements for pointer focus as well as keyboard focus, so it would sit there through ordinary typing. The caret and selection indicate focus in that interior instead. A non-editable region that earns a tab stop by scrolling — a capped payload, a read-only draft — draws the outline itself, having no caret to indicate focus with. A component that omits an outline on an editable surface records that choice as an entry in the focus inventory's editable declarations — naming the element and the reason — so the absence is a decision someone made rather than one nobody examined. The declaration is checked against what the element actually draws: a surface declared to draw nothing may not carry a focus class, and one declared to draw its own must.
- A live region is mounted for the lifetime of the surface that owns it, starts empty, and never has its liveness toggled on over text that is already rendered. Assistive technology announces mutations *inside* an existing live region; a region inserted into the DOM at the moment it first has text is usually silent, which makes a status that is never announced indistinguishable in code from one that is. The visible status text need not be the live region, and where both exist only one of them carries the role.
...
| A11Y-005 | Focus indication follows field identity, and live regions are mounted before they carry text. | `#accessibility-and-rendering-invariants` |
| A11Y-006 | Editable surfaces declare whether they draw a focus indicator. | `#accessibility-and-rendering-invariants` |
